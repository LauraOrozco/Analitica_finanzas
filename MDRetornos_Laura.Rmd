...
fluidRow(
  column(6,
  textInput("stock1", "Stock 1", "ATVI")),
  column(5,
  numericInput("w1", "Weight %", 25, min = 1, max = 100))
)  
fluidRow(
  column(6,
  textInput("stock2", "Stock 2", "ADBE")),
  column(5,
...
fluidRow(
  column(6,
  textInput("stock4", "Stock 4", "AMZN")),
  column(5,
  numericInput("w4", "Weight %", 20, min = 1, max = 100))
)
fluidRow(
  column(6,
  textInput("stock5", "Stock 5", "AAPL")),
  column(5,
...
###  Portafolio Monthly Return
```{r}
renderPlot({
  portfolio_returns_byhand() %>% 
    ggplot(aes(x = date, y = returns)) +
    geom_line(alpha = 0.25, binwidth = .01, fill = "cornflowerblue")
